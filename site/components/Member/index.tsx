...
export const Members = ({ children }) => {
  return <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    {children?.map((child, i) => {
      return <div key={`member-${i}`} className="flex justify-center">
        {child}
      </div>
    })}
  </div>
...
export const Member = (props: PropsWithChildren<MemberProps>) => {
  return <div className="card w-96 bg-base-100 shadow-xl">
    ...
    <div className="card-body">
      <h2 className="card-title text-center">
        {props.name}
        <div className="badge">{props.role}</div>
      </h2>
      <p>{props.intro}</p>
      {/* <div className="card-actions justify-end">
        <a href={`https://github.com/${props.github}`} className="btn">
